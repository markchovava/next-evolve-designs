...
export default function page() {
  ...
  async function searchData() {
    if(search == ''){
        ...
        try{
            const result = await AxiosClient.get(`category?search=${search}`, config)
            .then((response) => {
                setData(response.data.data)
  ...
  async function deleteData(id) {
    try{
      const result = await AxiosClient.delete(`category/${id}`, config)
      .then((response) => {
        getData()
      })
    } catch (error) {
      ...
      <section className='w-[100%] bg-slate-50'>
        <div className='mx-auto w-[90%]'>
            <ul className='py-2 flex items-center justify-start gap-2'>
              ...
              <li><BsChevronRight /></li>
              <li className='flex justify-start items-center'>
                <Link href='/admin/category' className='font-semibold'>
                  Category</Link>
              </li>
