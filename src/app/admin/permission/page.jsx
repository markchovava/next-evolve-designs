...
export default function page() {
  ...
  async function searchData() {
    if(search == ''){
        ...
        try{
            const result = await AxiosClient.get(`permission?search=${search}`, config)
            .then((response) => {
                setData(response.data.data)
  ...
  async function deleteData(id) {
    try{
      const result = await AxiosClient.delete(`permission/${id}`, config)
      .then((response) => {
        getData()
      })
    } catch (error) {
              ...
              data.map(item => (
                <div key={item.id} className="w-[100%] flex items-center justify-start py-3 border border-slate-200 ">
                    ...
                    <div className="w-[20%] p-3 border-l border-slate-300 flex justify-start items-center gap-3 text-xl">
                        <Link href={`/admin/permission/${item.id}`}> <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> </Link>
                        <Link href={`/admin/permission/edit/${item.id}`}> <MdEdit className='hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> </Link>
                        <button
