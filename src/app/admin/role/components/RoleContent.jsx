...
export default function RoleContent() {
    ...
    async function searchData() {
      if(search == ''){
          ...
          try{
              const result = await AxiosClient.get(`role?search=${search}`, config)
              .then((response) => {
                  setData(response.data.data)
    ...
    async function deleteData(id) {
      try{
        const result = await AxiosClient.delete(`role/${id}`, config)
        .then((response) => {
          getData()
        })
      } catch (error) {
            ...
            <div className='w-[100%] flex items-center justify-between h-auto pb-[1.2rem]'>
                <div className='lg:w-[40%] w-[70%] flex items-center justify-start gap-2'>
                    ...
                    <button 
                        ...
                        className='bg-gradient-to-br transition-all duration-150 ease-in rounded-lg px-7 py-3 bg-slate-500 text-white border hover:bg-gradient-to-br  hover:from-slate-500 hover:to-slate-700 hover:text-white '>
                        Search</button>
                </div>
                <div>
                    <Link
