                ...
                <div className="w-[100%] flex justify-center items-center">
                    <Swiper
                        ...
                        {usersData.data.map((item) => (
                            <SwiperSlide key={item.id} className='w-[100%] h-[25rem] rounded-xl border overflow-hidden bg-white border-white drop-shadow-xl'> 
                                <div className='w-[100%] h-[65%] bg-slate-600'>
                                    <img src={ item.image && baseURL + item.image } className="w-[100%] h-[100%]" />
                                </div>
                                <div className='w-[100%] h-[65%] p-[1rem]'>
                                    <h4 className='text-black font-bold text-xl leading-none pb-1'>{item.name ? item.name : 'Not added.'}</h4>
                                    <p className='italic pb-2'>{item.profession && item.profession }</p>
                                    <p>{item.bio ? item.bio.slice(0,30) : 'Not added.'}</p>
